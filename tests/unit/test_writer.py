...
from us_accidents_etl.load.writer import write_aggregations, write_dataset, write_filtered
...
def test_write_filtered_writes_to_correct_subpath(spark: SparkSession, etl_cfg: ETLConfig):
...
def test_write_aggregations_writes_all_five_dirs(spark: SparkSession, etl_cfg: ETLConfig):
    df = spark.createDataFrame([(1,)], ["x"])
    write_aggregations(df, df, df, df, df, etl_cfg)
    agg_base = os.path.join(etl_cfg.output_path, "agg")
    for name in ("severity_stats", "state_stats", "city_stats", "weather_stats", "day_night_stats"):
